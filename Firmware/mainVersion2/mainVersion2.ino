...
LiquidCrystal: https://www.sunfounder.com/learn/Sensor-Kit-v2-0-for-Arduino/lesson-1-display-by-i2c-lcd1602-sensor-kit-v2-0-for-arduino.html
...
bool password_addInput(char user_input)
{
  ...
  lcd.print( user_input );                                                                //Serial.println( user_input );                            
...
bool password_admin_process()
{
  bool isPassword_good = true;
  for(int i=0; i<5; i++)
  {
                                                                                                                                //Serial.print(password_enterred[i]); Serial.print(" - "); Serial.println(passwordAdmin_current[i]);
    if( password_enterred[i] != passwordAdmin_current[i] )
      isPassword_good = false;
  }
  if( isPassword_good == false )
  {
...
void update_userpw()
{
  ...
  lcd.clear(); lcd.setCursor(0,0); lcd.print("User PW Saved"); lcd.setCursor(0,1); lcd.print("Restarting...");                                   //Serial.println("User PW Saved"); Serial.println("Restarting...");   
...
void update_adminpw()
{
  for(int i=0; i<5; i++)
  {
    EEPROM.write(i+10, password_enterred[i]);
  }
  lcd.clear(); lcd.setCursor(0,0); lcd.print("Admin PW Saved"); lcd.setCursor(0,1); lcd.print("Restarting...");                                   //Serial.println("Admin PW Saved"); Serial.println("Restarting...");   
  buzzer_setState(BUZZER_ON); delay(10); buzzer_setState(BUZZER_OFF);
  delay(2000);
  asm volatile ("  jmp 0"); 
}
...
void timeoutMenu_update(int8_t offset)
{
  ...
  //generate menu
  if      (timeout_menuIndex == 0) { lcd.setCursor(0,1); lcd.print("15 minutes    "); }                                 //Serial.println("15 minutes    ");           }
  else if (timeout_menuIndex == 1) { lcd.setCursor(0,1); lcd.print("30 minutes    "); }                                 //Serial.println("30 minutes    ");           }
  else if (timeout_menuIndex == 2) { lcd.setCursor(0,1); lcd.print("45 minutes    "); }                                 //Serial.println("45 minutes    ");           }
  else if (timeout_menuIndex == 3) { lcd.setCursor(0,1); lcd.print("60 minutes    "); }                                 //Serial.println("60 minutes    ");           }
  else if (timeout_menuIndex == 4) { lcd.setCursor(0,1); lcd.print("90 minutes    "); }                                 //Serial.println("90 minutes    ");           }
  else if (timeout_menuIndex == 5) { lcd.setCursor(0,1); lcd.print("120 minutes   "); }                                //Serial.println("120 minutes    ");          }
}

void timeoutMenu_select()
{
  lcd.clear(); lcd.setCursor(0,0); lcd.print("Time Updated"); lcd.setCursor(0,1); lcd.print("Restarting...");
                                                                                                                      //Serial.println("Time Updated"); Serial.println("Restarting...");
...
void adminMenu_update(int8_t offset)
{
  ...
  //deal with min/max menu
  if      (admin_menuIndex<0) { admin_menuIndex = 3; }
  else if (admin_menuIndex>3) { admin_menuIndex = 0; }
  //generate menu
  if      (admin_menuIndex == 0) { lcd.setCursor(0,1); lcd.print("New User PW");      }                                  //Serial.println("New User PW");                         }
  else if (admin_menuIndex == 1) { lcd.setCursor(0,1); lcd.print("New Admin PW");     }                                 //Serial.println("New Admin PW");                         }
  else if (admin_menuIndex == 2) { lcd.setCursor(0,1); lcd.print("Change Timout   "); }                            //Serial.println("Change Timout   ");                         }
  else if (admin_menuIndex == 3) { lcd.setCursor(0,1); lcd.print("Exit            "); }                            //Serial.println("Exit            ");                         }
}
...
void adminMenu_select()
{
  ...
  {
    lcd.clear(); lcd.setCursor(0,0); lcd.print("Enter New PW:"); lcd.setCursor(0,1);                                    //Serial.println("Enter New PW:");
    CURRENT_STATE = STATE_NEW_PW;
  }
  else if(admin_menuIndex == 1)
  {
    lcd.clear(); lcd.setCursor(0,0); lcd.print("Enter New PW"); lcd.setCursor(0,1);                                     //Serial.println("Enter New PW:");
    CURRENT_STATE = STATE_NEW_ADMIN_PW;
  }
  else if(admin_menuIndex == 2)
  {
    CURRENT_STATE = STATE_NEW_TIMEOUT;
  }
  else if(admin_menuIndex == 3)
  {
    asm volatile ("  jmp 0"); 
  }
}
...
void lcd_showTimeLeft()
{
  ...
  {
    lcd.setCursor(0,1);
    lcd.print("Time Left: ");                                               //Serial.println("Time Left: ");
    lcd.print(powerOff_countdown_inSeconds);                                //Serial.println(powerOff_countdown_inSeconds);
  ...
  */
  ...
  for(int i=0; i<5; i++)
  {
    password_current[i] = EEPROM.read(i);     //load stored password
                                                                                            //Serial.print(password_current[i]);
  }
                                                                                            //Serial.println("");
  
  //load timeout
  power_defaultTimeout = EEPROM.read(5);
                                                                                            //Serial.print(power_defaultTimeout);
                                                                                            //Serial.println("");

  //load admin password
  for(int i=0; i<5; i++)
  {
    passwordAdmin_current[i] = EEPROM.read(i+10);
                                                                                            //Serial.print(passwordAdmin_current[i]);
  }
                                                                                            //Serial.println("");
  ...
  lcd.backlight();
  lcd.setCursor(0,0);
  lcd.print("Loading");                                                     //Serial.println("Loading");
  lcd.setCursor(0,1);
  for(int i=0; i<16; i++)
  {
    lcd.print(char(255));                                                   //Serial.println(char(255));
    delay(5);
  }
  delay(500);
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Helios Machine");                                              //Serial.println("Helios Machine");
  lcd.setCursor(0, 1);
  lcd.print("Control - V1.0");                                              //Serial.println("Control - V1.0");
  delay(1000);

  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("by   Black Cat");                                              //Serial.println("by   Black Cat");
  lcd.setCursor(0, 1);
  lcd.print("     Robotics");                                               //Serial.println("     Robotics");
  delay(1000);
}
...
void loop()
{
  ...
  switch(CURRENT_STATE)
  {
    case STATE_START:
      //Execution
      lcd.clear(); lcd.setCursor(0,0); lcd.print("Enter Password");                                                           //Serial.println("Enter Password");
    ...
    case STATE_INPUT:
      ...
      { 
        ...
        {
          lcd.clear(); lcd.setCursor(0,0); lcd.print("Admin Login"); lcd.setCursor(0,1); lcd.print("PW:");                    //Serial.println("Admin Login"); Serial.println("PW:");
    ...
    case STATE_PROCESS_ADMIN_PW:
      //Execution
      isPasswordCorrect = false;
      isPasswordCorrect = password_admin_process();
      //Transition Out
      if( isPasswordCorrect == true )
    ...
    case STATE_PW_GOOD:
      //Execution
      lcd.clear(); lcd.setCursor(0,0); lcd.print("Password OK"); lcd.setCursor(0,1); lcd.print("Powering On");                                        //Serial.println("Password OK");  Serial.println("Powering On");
    ...
    case STATE_ADMIN_PW_GOOD:
      //Execution
      lcd.clear(); lcd.setCursor(0,0); lcd.print("Password OK"); lcd.setCursor(0,1); lcd.print("Admin Mode");                                         //Serial.println("Password OK");  Serial.println("Admin Mode");
      buzzer_scheduleBeep(2, 50);
      //wait state
      wait_timeStamp = millis();
    ...
    case STATE_ADMINMODE:
      ...
      if(lcd_updateCompleted != LCD_DONE)
      {
        ...
        lcd.clear(); lcd.setCursor(0,0); lcd.print("<- *  MENU  # ->");                                                                               //Serial.println("<- *  MENU  # ->");
        adminMenu_update(0); 
      }
      key = keypad.getKey();
      if(key) //de-bouncer
    ...
    case STATE_PW_BAD:
      //Execution
      lcd.clear(); lcd.setCursor(0,0); lcd.print("Bad Password");                                                                                      //Serial.println("Bad Password");
    ...
    case STATE_ON:
      ...
      {
        lcd_updateCompleted = LCD_DONE;
        lcd.clear(); lcd.setCursor(0,0); lcd.print("# to Shutdown"); lcd.setCursor(0,1);                                                               //Serial.println("# to Shutdown"); 
      }
      lcd_showTimeLeft();
      key = keypad.getKey();
      if(key) //de-bouncer
    ...
    case STATE_OFF:
      //Execution
      lcd.clear(); lcd.setCursor(0,0); lcd.print("Power Off");                                                                                          //Serial.println("Power Off");
      relay_setState(RELAY_OFF);
      digitalWrite(LED_GREEN_PIN, LOW);
      buzzer_scheduleBeep(2, 50);
      //go into wait state
    ...
    case STATE_NEW_PW:
      ...
      { 
        if(key=='*' || key=='#')
        {
          lcd.clear(); lcd.setCursor(0,0); lcd.print("Cancelled");                                                                                     //Serial.println("Cancelled"); 
          buzzer_setState(BUZZER_ON); delay(10); buzzer_setState(BUZZER_OFF);
          delay(2000);
          asm volatile ("  jmp 0"); 
        }
    ...
    case STATE_NEW_TIMEOUT:
      ...
      {
        lcd_updateCompleted = LCD_DONE;
        lcd.clear(); lcd.setCursor(0,0); lcd.print("<- *  TIME  # ->");                                                                              //Serial.println("<- *  TIME  # ->");
    ...
    default:
      lcd.clear(); lcd.setCursor(0,0); lcd.print("Error: "); lcd.setCursor(0,1); lcd.print("State Unknown");                                        //Serial.println("Error: "); Serial.println("State Unknown");              
      break;
  }
}
